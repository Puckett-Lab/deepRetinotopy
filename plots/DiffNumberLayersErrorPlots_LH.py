                ...
                for j in range(len(a['Predicted_values'])):
                    ...
                    for i in range(len(a['Predicted_values'])):
                        ...
                        if i == j:
                            ...
                            pred = np.reshape(
                                np.array(a['Predicted_values'][i]), (-1, 1))
                            measured = np.reshape(np.array(a['Measured_values'
                                                             ''][j]), (-1, 1))
                            ...
                            # Computing delta theta, angle between vector
                            # defined predicted value and empirical value
                            # same subj
                        ...
                        if i != j:
                            ...
                            pred = np.reshape(
                                np.array(a['Predicted_values'][i]), (-1, 1))
                            pred2 = np.reshape(
                                np.array(a['Predicted_values'][j]), (-1, 1))
                            measured = np.reshape(
                                np.array(a['Measured_values'][j]), (-1, 1))
                            measured2 = np.reshape(
                                np.array(a['Measured_values'][i]), (-1, 1))
                            ...
                            # # Computing delta theta, angle between vector
                            # defined predicted i and empirical j map
                            # theta = smallest_angle(pred,measured)
                            # theta_across_temp.append(theta)
                            #
                            # # Computing delta theta, angle between vector
                            # defined measured i versus measured j
                    ...
                    # theta_acrosssubj.append(np.mean(theta_across_temp,
                    # axis=0))
                    # theta_acrosssubj_emp.append(np.mean(
                    # theta_emp_across_temp, axis=0))
                    theta_acrosssubj_pred.append(
                        np.mean(theta_pred_across_temp, axis=0))

                R2_thr = np.mean(R2_thr, axis=0)

                # mean_theta_acrosssubj=np.mean(np.array(theta_acrosssubj),
                # axis=0)
                mean_theta_withinsubj = np.mean(np.array(theta_withinsubj),
                                                axis=0)
                # mean_theta_acrosssubj_emp=np.mean(np.array(
                # theta_acrosssubj_emp),axis=0)
            ...
            for j in range(len(a['Predicted_values'])):
                ...
                for i in range(len(a['Predicted_values'])):
                    ...
                    if i == j:
                        ...
                        # pred_shuffled = np.array(pred_shuffled) * (np.pi /
                        # 180)
                        ...
                        # Computing delta theta, angle between vector
                        # defined predicted value and empirical value same subj
                        theta = smallest_angle(pred, measured)
                        theta_withinsubj.append(theta)

                        # theta_shuffled = smallest_angle(pred_shuffled,
                        # measured)
                ...
                theta_acrosssubj_emp.append(
                    np.mean(theta_emp_across_temp, axis=0))
                theta_acrosssubj_pred.append(
                    np.mean(theta_pred_across_temp, axis=0))
