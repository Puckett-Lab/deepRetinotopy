...
a = torch.load(
    ...
    map_location='cpu')
pred[final_mask_R == 1] = np.reshape(np.array(a['Predicted_values'][8]),
                                     (-1, 1))
measured[final_mask_R == 1] = np.reshape(np.array(a['Measured_values'][8]),
                                         (-1, 1))
...
# .32k_fs_LR.surf.gii'),surf_map=np.reshape(pred[0:32492],(-1)),
# bg_map=background,cmap='gist_rainbow_r',symmetric_cbar=False,vmax=361,
# view='medial',avg_method='median',threshold=361,
# output_file='medial_305_3D.svg')
# plotting.show()
