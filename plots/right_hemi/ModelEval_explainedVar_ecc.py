...
# ecc_average = np.load('./../output/AverageEccentricityMap_RH.npz')['list']
        ...
        for j in range(len(predictions['Predicted_values'])):
            for i in range(len(predictions['Predicted_values'])):
                if i == j:
                    ...
                    # values
                    corr_theta = np.corrcoef(np.reshape(pred[mask>1],(-1)), np.reshape(measured[mask>1],(-1)))[0,1]
                    theta_withinsubj.append(corr_theta)

        # np.savez('./output/corr_higherOrder_RH_ecc_averageMap.npz',
        #     list=np.reshape(theta_withinsubj,(10,-1)))
        # np.savez('./output/corr_higherOrder_RH_ecc_Model.npz',
        #          list=np.reshape(theta_withinsubj, (10, -1)))
...
for m in range(len(models)):
    ...
    # np.savez('./output/corr_earlyVisualCortex_RH_ecc_averageMap.npz',
    #          list=np.reshape(theta_withinsubj, (10, -1)))
    # np.savez('./output/corr_earlyVisualCortex_RH_ecc_Model.npz',
    #          list=np.reshape(theta_withinsubj, (10, -1)))
...
corr_EarlyVisualCortex_model = np.reshape(np.array(
    np.load('./../output/corr_earlyVisualCortex_RH_ecc_Model.npz')['list']),
    (10, -1))
corr_higherOrder_model = np.reshape(np.array(
    np.load('./../output/corr_higherOrder_RH_ecc_Model.npz')['list']), (10, -1))

corr_EarlyVisualCortex_average = np.reshape(np.array(
    np.load('./../output/corr_earlyVisualCortex_RH_ecc_averageMap.npz')[
        'list']), (10, -1))
corr_higherOrder_average = np.reshape(np.array(
    np.load('./../output/corr_higherOrder_RH_ecc_averageMap.npz')['list']),
    (10, -1))
