...
for m in range(len(models)):
    ...
    for i in range(len(a['Predicted_values'])):
        ...
        if i != j:
            ...
            # Rescaling polar angles to match the right visual field (left
            # hemisphere)
            minus = pred > 180
            sum = pred < 180
            pred[minus] = pred[minus] - 180
            ...
            #
            # # Computing delta theta, angle between vector defined measured
            # i versus measured j
...
view = plotting.view_surf(
    ...
    cmap='Blues', black_bg=False, symmetric_cmap=False,
    threshold=threshold * 10, vmax=(75 + threshold) * 10)
