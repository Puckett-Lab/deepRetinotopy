...
for k in range(len(visual_areas)):
    ...
    data_tmp = np.concatenate([[mean_across[0], len(mean_across[0])*[models[0]], len(mean_across[0])*['Between predicted maps'],len(mean_across[0])*['Cluster'+str(k+1)]],
                           [mean_across[1], len(mean_across[1])*[models[1]], len(mean_across[1])*['Between predicted maps'],len(mean_across[1])*['Cluster'+str(k+1)]],
                           [mean_across[2], len(mean_across[2])*[models[2]], len(mean_across[2])*['Between predicted maps'],len(mean_across[2])*['Cluster'+str(k+1)]],
                           [mean_delta[0], len(mean_across[0])*[models[0]], len(mean_across[0])*['Between predicted map and ground truth'],len(mean_across[0])*['Cluster'+str(k+1)]],
                           [mean_delta[1], len(mean_across[1])*[models[1]], len(mean_across[1])*['Between predicted map and ground truth'],len(mean_across[1])*['Cluster'+str(k+1)]],
                           [mean_delta[2], len(mean_across[2])*[models[2]], len(mean_across[2])*['Between predicted map and ground truth'],len(mean_across[2])*['Cluster'+str(k+1)]]], axis=1)
    data.append(data_tmp)
    # df = pd.DataFrame(columns=['$\Delta$$\t\Theta$', 'models', 'label','cluster'], data=data_tmp.T)
...
df = pd.DataFrame(columns=['$\Delta$$\t\Theta$', 'models', 'label','cluster'], data=data.T)
df['$\Delta$$\t\Theta$'] = df['$\Delta$$\t\Theta$'].astype(float)
palette = ['dimgray', 'lightgray']
# ax = sns.boxplot(y='$\Delta$$\t\Theta$', x='models', order=models, hue='label', data=df, palette=palette,showfliers=False)
ax = sns.catplot(y='$\Delta$$\t\Theta$', x='models',col='cluster', order=models, hue='label', data=df, palette=palette,join=False,dodge=True,ci=95,kind='point')
...
data_tmp_2 = np.concatenate([[mean_across_2[0], len(mean_across_2[0])*[models[0]], len(mean_across_2[0])*['Between predicted maps'],len(mean_across[0])*['Early visual cortex']],
                           [mean_across_2[1], len(mean_across_2[1])*[models[1]], len(mean_across_2[1])*['Between predicted maps'],len(mean_across[1])*['Early visual cortex']],
                           [mean_across_2[2], len(mean_across_2[2])*[models[2]], len(mean_across_2[2])*['Between predicted maps'],len(mean_across[2])*['Early visual cortex']],
                           [mean_delta_2[0], len(mean_across_2[0])*[models[0]], len(mean_across_2[0])*['Between predicted map and ground truth'],len(mean_across[0])*['Early visual cortex']],
                           [mean_delta_2[1], len(mean_across_2[1])*[models[1]], len(mean_across_2[1])*['Between predicted map and ground truth'],len(mean_across[1])*['Early visual cortex']],
                           [mean_delta_2[2], len(mean_across_2[2])*[models[2]], len(mean_across_2[2])*['Between predicted map and ground truth'],len(mean_across[2])*['Early visual cortex']]], axis=1)

data.append(data_tmp_2)
