...
# pre_transform=pre_transform,n_examples=181,prediction='polarAngle',
# myelination=True,hemisphere='Left')
...
for k in range(len(twin_MZ)):
    ...
    similarity_PA_pred['MZ'].append(
        matrix_PA_pred[twin_MZ[k][0], twin_MZ[k][1]])
    ...
    similarity_PA_pred['DZ'].append(
        matrix_PA_pred[twin_DZ[k][0], twin_DZ[k][1]])
...
features_title = ['Polar Angle - Ground truth', 'Polar Angle - Predicted',
                  'Curvature', 'Myelin']
fig = plt.figure(figsize=(10, 3))
for i in range(len(features_read)):
    print('Mean MZ:' + str(np.mean(
        vars()['similarity_' + features_read[i]]['MZ'])) + ', Mean DZ:' + str(
        np.mean(vars()['similarity_' + features_read[i]][
                    'DZ'])) + ' Mean UN:' + str(
        np.mean(vars()['similarity_' + features_read[i]]['UN'])))
    data = np.concatenate([[vars()['similarity_' + features_read[i]]['MZ'],
                            len(vars()['similarity_' + features_read[i]][
                                    'MZ']) * ['MZ']],
                           [vars()['similarity_' + features_read[i]]['DZ'],
                            len(vars()['similarity_' + features_read[i]][
                                    'DZ']) * ['DZ']],
                           [vars()['similarity_' + features_read[i]]['UN'],
                            len(vars()['similarity_' + features_read[i]][
                                    'UN']) * ['UN']]], axis=1)
    print(data)
    ...
    ax = sns.pointplot(y='Correlation', x='Zygosity', hue='Zygosity', data=df,
                       join=False, dodge=True, ci=95, palette='colorblind')
    plt.axhline(
        np.mean(np.mean(vars()['similarity_' + features_read[i]]['UN'])),
        linestyle='--', color='gray')
    ax.set_title(features_title[i])
    plt.ylim(-0.1, 1)
    legend = plt.legend()

# plt.boxplot([similarity_PA['MZ'],similarity_PA['DZ'],similarity_PA['UN']],
# [1,2,3],labels=['MZ','DZ','UN'])
plt.show()
