...
model.load_state_dict(torch.load(
    ...
    '/model4_nothresh_rotated_12layers_smoothL1lossR2_curvnmyelin_ROI1_k25_batchnorm_dropout010_5.pt',
    map_location='cpu'))
...
#         MeanAbsError += MAE
#     test_MAE = MeanAbsError / len(test_loader)
#     output = {'Predicted_values': y_hat, 'Measured_values': y, 'MAE':
#     test_MAE,'Shuffled_myelin':myelin,'Shuffled_curv':curv}
#     return output
#
#
# evaluation = test()
...
# Evaluating trained model on the test dataset (shuffled curvature and
# empirical myelin values)
# def test():
#     model.eval()
#     MeanAbsError=0
#     y=[]
...
#         #Shuffling curv
#         data.x.transpose(0,1)[0]=data.x.transpose(0,1)[0][torch.randperm(
#         3267)]
#         pred=model(data.to(device)).detach()
#         y_hat.append(pred)
#         y.append(data.to(device).y.view(-1))
...
# evaluation=test()
# torch.save({'Predicted_values':evaluation['Predicted_values'],
# 'Measured_values':evaluation['Measured_values']},osp.join(osp.dirname(
# osp.realpath(__file__)),'testing_shuffled-curv.pt'))


# Evaluating trained model on the test dataset (empirical curvature and
# shuffled myelin values)
# def test():
#     model.eval()
#     MeanAbsError = 0
#     y = []
...
#     for data in dev_loader:
#         # Shuffling myelin
#         data.x.transpose(0, 1)[1] = data.x.transpose(0, 1)[1][
#         torch.randperm(3267)]
#         pred = model(data.to(device)).detach()
#         y_hat.append(pred)
#         y.append(data.to(device).y.view(-1))
...
#     'Shuffled_myelin':x, 'MAE': test_MAE}
#     return output
#
#
# evaluation = test()
# torch.save({'Predicted_values': evaluation['Predicted_values'],
# 'Measured_values': evaluation['Measured_values']},osp.join(osp.dirname(
# osp.realpath(__file__)), 'testing_shuffled-myelin.pt'))
...
# 'Measured_values': evaluation['Measured_values']},
#            osp.join(osp.dirname(osp.realpath(__file__)),'testset_results',
#            'testset-constant_Model3_PA_LH.pt'))
