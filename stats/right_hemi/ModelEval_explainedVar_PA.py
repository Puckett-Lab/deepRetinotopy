...
# PA_average = np.load('./../../plots/AveragePolarAngleMap_RH.npz')['list']
    ...
    for j in range(len(a['Predicted_values'])):
        for i in range(len(a['Predicted_values'])):
            if i == j:
                ...
                # # Uncomment the line bellow to evaluate the performance of the
                # # average map
                # pred = np.reshape(np.array(PA_average), (-1, 1))
...
corr_EarlyVisualCortex_model = np.reshape(np.array(
    np.load('./../output/corr_earlyVisualCortex_RH_PA_Model.npz')['list']),
    (10, -1))
corr_higherOrder_model = np.reshape(np.array(
    np.load('./../output/corr_higherOrder_RH_PA_Model.npz')['list']), (10, -1))

corr_EarlyVisualCortex_average = np.reshape(np.array(
    np.load('./../output/corr_earlyVisualCortex_RH_PA_averageMap.npz')[
        'list']), (10, -1))
corr_higherOrder_average = np.reshape(np.array(
    np.load('./../output/corr_higherOrder_RH_PA_averageMap.npz')['list']),
    (10, -1))
