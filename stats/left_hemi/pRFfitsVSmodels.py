...
def PA_difference_fits(model):
    ...
    Output: .npz files in ./../output named:
        'ErrorPerParticipant_PA_LH_WangParcels_' + str(model) + '_1-8_fit2(3).npz'
        'ErrorPerParticipant_PA_LH_EarlyVisualCortex_' + str(model) + '_1-8_fit2(3).npz'
        'ErrorPerParticipant_PA_LH_dorsalV1-3_' + str(model) + '_1-8_fit2(3).npz'
    ...
    for k in range(len(models)):
        ...
        for j in range(len(predictions['Predicted_values'])):
            for i in range(len(predictions['Predicted_values'])):
                if i == j:
                    ...
                    # theta = smallest_angle(measured_fit1[eccentricity_mask],
                    #                        measured[eccentricity_mask])
        ...
        # np.savez('./../output/ErrorPerParticipant_PA_LH_WangParcels_fit1_1-8_' +
        #          models[k] + '.npz',
        #          list=np.reshape(theta_withinsubj, (10, -1)))

        np.savez('./../output/ErrorPerParticipant_PA_LH_WangParcels_' + str(model) + '_1-8_' +
                 models[k] + '.npz',
                 list=np.reshape(theta_withinsubj, (10, -1)))
    ...
    for m in range(len(models)):
        ...
        # np.savez('./../output/ErrorPerParticipant_PA_LH_EarlyVisualCortex_fit1_1-8_' +
        #              models[m] + '.npz', list=np.reshape(theta_withinsubj, (10, -1)))

        np.savez('./../output/ErrorPerParticipant_PA_LH_EarlyVisualCortex_' + str(model) + '_1-8_' +
                     models[m] + '.npz', list=np.reshape(theta_withinsubj, (10, -1)))
    ...
    for m in range(len(models)):
        ...
        # np.savez('./../output/ErrorPerParticipant_PA_LH_dorsalV1-3_fit1_1-8_' +
        #              models[m] + '.npz',
        #          list=np.reshape(theta_withinsubj, (10, -1)))
