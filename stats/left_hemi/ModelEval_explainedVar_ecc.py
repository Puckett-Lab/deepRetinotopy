...
# # Uncomment for predictions based on the average map
# ecc_average = np.load('./../../plots/output/AverageEccentricityMap_LH.npz')['list']
for k in range(len(visual_areas)):
    ...
    for m in range(len(models)):
        ...
        # np.savez('./../output/corr_higherOrder_LH_ecc_averageMap.npz',
        #          list=np.reshape(theta_withinsubj, (10, -1)))
        # np.savez('./../output/corr_higherOrder_LH_ecc_Model.npz',
        #          list=np.reshape(theta_withinsubj, (10, -1)))
...
mean_delta_2 = []
for m in range(len(models)):
    a = torch.load('./../../testset_results/left_hemi/testset-' +
            models[m] + '_Model4_ecc_LH.pt', map_location='cpu')
    ...
    for j in range(len(a['Predicted_values'])):
        for i in range(len(a['Predicted_values'])):
            if i == j:
                ...
                # values
                corr_theta = np.corrcoef(np.reshape(pred[mask > 1], (-1)),
                                    np.reshape(measured[mask > 1], (-1)))[0, 1]
                theta_withinsubj.append(corr_theta)

    # np.savez('./../output/corr_earlyVisualCortex_LH_ecc_Model.npz',
    #          list=np.reshape(theta_withinsubj, (10, -1)))
    # np.savez('./../output/corr_earlyVisualCortex_LH_ecc_averageMap.npz',
    #          list=np.reshape(theta_withinsubj, (10, -1)))
    corr_theta_withinsubj = np.mean(np.array(theta_withinsubj), axis=0)
...
corr_EarlyVisualCortex_model = np.reshape(np.array(
    np.load('./../output/corr_earlyVisualCortex_LH_ecc_Model.npz')['list']),
    (10, -1))
corr_higherOrder_model = np.reshape(np.array(
    np.load('./../output/corr_higherOrder_LH_ecc_Model.npz')['list']),
    (10, -1))

corr_EarlyVisualCortex_average = np.reshape(np.array(
    np.load('./../output/corr_earlyVisualCortex_LH_ecc_averageMap.npz')[
        'list']), (10, -1))
corr_higherOrder_average = np.reshape(np.array(
    np.load('./../output/corr_higherOrder_LH_ecc_averageMap.npz')['list']),
    (10, -1))
