...
while i<6:
    ...
    model=Net().to(device)
    model.load_state_dict(torch.load('/home/uqfribe1/PycharmProjects/DEEP-fMRI/model4_nothresh_rotated_12layers_smoothL1lossR2_curvnmyelin_ROI1_k25_batchnorm_dropout010_notwin10_'+str(i)+'.pt',map_location='cpu'))
    #
    # def test():
    #     model.eval()
    #     MeanAbsError = 0
    ...
    evaluation = test()
    torch.save({'Predicted_values': evaluation['Predicted_values'], 'Measured_values': evaluation['Measured_values']},
               osp.join(osp.dirname(osp.realpath(__file__)),'testset_results', 'testset-pred_Model'+str(i)+'_PA_LH_notwin10.pt'))
    i=i+1