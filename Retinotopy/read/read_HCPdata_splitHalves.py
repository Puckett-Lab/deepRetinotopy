...
def read_HCP(path, Hemisphere=None, index=None, surface=None, threshold=None,
    ...
    eccentricity = \
        scipy.io.loadmat(osp.join(path, 'cifti_eccentricity_fit' + str(
            fit) + '_all.mat'))[
            'cifti_eccentricity']
    polarAngle = scipy.io.loadmat(osp.join(path,
                                           'cifti_polarAngle_fit' + str(
                                               fit) + '_all.mat'))[
        'cifti_polarAngle']
    ...
    if Hemisphere == 'Right':
        ...
        eccentricity_values = torch.tensor(np.reshape(
            eccentricity['x' + subjects[index] + '_fit'+str(fit)+'_eccentricity_msmall'][
                0][0][
            number_hemi_nodes:number_cortical_nodes].reshape(
                (number_hemi_nodes))[visual_mask_R == 1], (-1, 1)),
            dtype=torch.float)
        polarAngle_values = torch.tensor(np.reshape(
            polarAngle['x' + subjects[index] + '_fit'+str(fit)+'_polarangle_msmall'][0][
                0][
            number_hemi_nodes:number_cortical_nodes].reshape(
                (number_hemi_nodes))[visual_mask_R == 1], (-1, 1)),
            dtype=torch.float)
