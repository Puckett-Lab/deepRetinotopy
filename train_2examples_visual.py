...
class Net(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x=F.elu(self.conv2(x,edge_index,pseudo)).view(-1)-1
        return x
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        print(len(data.y.view(-1)))
        print(len(data.x.view(-1)))
...
for epoch in range(1, 1000):
    ...
    if epoch%50==0:
        torch.save({'Epoch':epoch,'Predicted_values':test_output['Predicted_values'],'Measured_values':test_output['Measured_values'],'Loss':loss,'Dev_MAE':test_output['MAE']},osp.join(osp.dirname(osp.realpath(__file__)), 'model_2examples_visual_noTnorm_output_epoch'+str(epoch)+'.pt'))


#Saving the model's learned parameter and predicted/y values
torch.save(model.state_dict(),osp.join(osp.dirname(osp.realpath(__file__)), 'model_2examples_visual_noTNorm.pt'))
