...
train_dataset=Retinotopy(path,'Train', transform=T.Cartesian(),pre_transform=pre_transform,n_examples=181,prediction='pRFsize',myelination=True,hemisphere='Right')
dev_dataset=Retinotopy(path,'Development', transform=T.Cartesian(),pre_transform=pre_transform,n_examples=181,prediction='pRFsize',myelination=True,hemisphere='Right')
...
for epoch in range(1, 201):
    ...
    if epoch%25==0:
        torch.save({'Epoch':epoch,'Predicted_values':test_output['Predicted_values'],'Measured_values':test_output['Measured_values'],'R2':test_output['R2'],'Loss':loss,'Dev_MAE':test_output['MAE']},osp.join(osp.dirname(osp.realpath(__file__)),'..','output','model4_nothresh_RH_pRFsize_12layers_smoothL1loss_curvnmyelin_ROI1_k25_batchnorm_dropout010_5_output_epoch'+str(epoch)+'.pt'))
    if test_output['MAE']<=0.3: #MeanAbsError from Benson2014
        break


#Saving the model's learned parameter and predicted/y values
torch.save(model.state_dict(),osp.join(osp.dirname(osp.realpath(__file__)),'..','output','model4_nothresh_RH_pRFsize_12layers_smoothL1loss_curvnmyelin_ROI1_k25_batchnorm_dropout010_5.pt'))
