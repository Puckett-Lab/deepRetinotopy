...
for epoch in range(1, 301):
    ...
    if epoch%25==0:
        torch.save({'Epoch':epoch,'Predicted_values':test_output['Predicted_values'],'Measured_values':test_output['Measured_values'],'R2':test_output['R2'],'Loss':loss,'Dev_MAE':test_output['MAE']},osp.join(osp.dirname(osp.realpath(__file__)),'..','output','model4_nothresh_RH_12layers_smoothL1lossR2_curvnmyelin_ROI1_k25_batchnorm_dropout010_4_output_epoch'+str(epoch)+'.pt'))
        # torch.save(model.state_dict(), osp.join(osp.dirname(osp.realpath(__file__)), '..', 'output',
        #                                         'modelparameters_nothresh_RH_12layers_smoothL1lossR2_curvnmyelin_ROI1_k25_batchnorm_dropout010_4_epoch' + str(
        #                                             epoch) + '.pt'))
...
torch.save(model.state_dict(),osp.join(osp.dirname(osp.realpath(__file__)),'..','output','model4_300_nothresh_RH_12layers_smoothL1lossR2_curvnmyelin_ROI1_k25_batchnorm_dropout010_4.pt'))
