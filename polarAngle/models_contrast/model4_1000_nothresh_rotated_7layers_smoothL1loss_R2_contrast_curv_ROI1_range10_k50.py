...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv5 = SplineConv(32, 16, dim=3, kernel_size=50, norm=False)
        self.conv6=SplineConv(16,8,dim=3,kernel_size=50,norm=False)
        self.conv7 = SplineConv(8, 1, dim=3, kernel_size=50, norm=False)
    ...
    def forward(self, data):
        ...
        x=F.elu(self.conv7(x,edge_index,pseudo)).view(-1)
        return x
...
for epoch in range(1, 1001):
    ...
    if epoch%1000==0:
        torch.save({'Epoch':epoch,'Predicted_values':test_output['Predicted_values'],'Measured_values':test_output['Measured_values'],'R2':test_output['R2'],'Loss':loss,'Dev_MAE':test_output['MAE']},osp.join(osp.dirname(osp.realpath(__file__)),'..','output','model4_nothresh_rotated_7layers_smoothL1lossR2_contrast_curv_ROI1_range10_k50_output_epoch'+str(epoch)+'.pt'))
    if test_output['MAE']<=10.94: #MeanAbsError from Benson2014
        break


#Saving the model's learned parameter and predicted/y values
torch.save(model.state_dict(),osp.join(osp.dirname(osp.realpath(__file__)),'..','output','model4_nothresh_rotated_7layers_smoothL1lossR2_contrast_curv_ROI1_range10_k50.pt'))
