...
def roi(list_of_labels):
    ...
    index_L_exclude = [i for i, j in enumerate(final_mask_L) if j == 0]
    index_R_exclude = [i for i, j in enumerate(final_mask_R) if j == 0]

    index_L_mask = [i for i, j in enumerate(final_mask_L) if j == 1]
    index_R_mask = [i for i, j in enumerate(final_mask_R) if j == 1]

    return final_mask_L,final_mask_R,index_L_mask,index_R_mask,index_L_exclude,index_R_exclude
