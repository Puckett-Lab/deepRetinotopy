...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = SplineConv(1, 4, dim=3, kernel_size=5, norm=False)
        self.conv2 = SplineConv(4, 8, dim=3, kernel_size=5, norm=False)
        self.conv3 = SplineConv(8, 1, dim=3, kernel_size=5, norm=False)
    ...
    def forward(self, data):
        ...
        x = F.elu(self.conv3(x, edge_index, pseudo)).view(-1)
...
def test():
    ...
    for data in dev_loader:
        ...
        MAE=torch.mean(abs(data.to(device).y.view(-1)[threshold]-pred[threshold]))
        MeanAbsError=MeanAbsError+MAE
...
for epoch in range(1, 100):
    ...
    if epoch%50==0:
        torch.save({'Epoch':epoch,'Predicted_values':test_output['Predicted_values'],'Measured_values':test_output['Measured_values'],'Loss':loss,'Dev_MAE':test_output['MAE']},osp.join(osp.dirname(osp.realpath(__file__)), 'model2_output_epoch'+str(epoch)+'.pt'))


#Saving the model's learned parameter and predicted/y values
torch.save(model.state_dict(),osp.join(osp.dirname(osp.realpath(__file__)), 'model2.pt'))
